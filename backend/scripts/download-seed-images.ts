...
const main = async () => {
	...
	const seedImagesDir = path.join(__dirname, '..', 'server', 'db', 'seed-images');
	...
	if (!existsSync(profilesDir)) await mkdir(profilesDir, { recursive: true });
	...
	for (let i = 0; i < IMAGE_CATEGORIES.profiles.length; i++) {
		const query = IMAGE_CATEGORIES.profiles[i];
		console.log(`  🔍 Searching: "${query}"`);
	...
	console.log(
		`📊 Total images: ${IMAGE_CATEGORIES.posts.length + IMAGE_CATEGORIES.quizzes.length + IMAGE_CATEGORIES.profiles.length}`,
	);
